...
//dont touch please
document.write('<canvas id="cv" width="'+cvwidth+'" height="'+cvheight+'" style="border:'+cvborder+'px red solid;"></canvas>');var ctx=document.getElementById("cv").getContext("2d");function box(posX,posY,width,height,color,strokecolor,strokewidth){ctx.beginPath();ctx.rect(posX,posY,width,height);ctx.lineWidth= strokewidth;ctx.strokeStyle= strokecolor;ctx.fillStyle = color;ctx.stroke();ctx.fill();}
